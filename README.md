# Time Calculator
时间计算器 | 适用平台：Windows、安卓

## 概述
做了一个普通的计算器时分秒转换没有的，更方便 用来计算时间的工具。遇到一些：需要直接计算(加/减)一个比较大的时间的数 或者 需要连续计算(加/减)多次...等等的场景时，可以使用此工具。

## 下载安装与使用

### Windows
在 Release（发行）中下载`TimeCalculator_版本号.exe`

该程序采用.NET Framework框架制作，若计算机没安装过.NET Framework，需要把 Release 中的`dotNET_Framework_4.7.2_Setup.exe`也下载然后把.Net安装上。

打开`TimeCalculator_版本号.exe`即可使用。

### 安卓（Android）
1. 在 Release （发行）中下载`Time_Calculator_版本号.html`，用浏览器打开这个网页文件来使用。
2. 下载`Time_Calculator_版本号.apk`文件，安装之后使用。（apk安装包的仅1.1.0版本开始有，1.0.0版本的没保留下来）

>### 历史
>2025年，Python和html网页版的时间计算器是最先做做出来。
>
>2026年，使用Python改用.NET来做，相当于是重制了，所以程序内信息写的是2026的。至于网页版的话是为适配手机而做的，最新版也跟随`用.NET做的计算器功能添加`更新了（里面版权信息写的年份是2025的话是为了保留那次制作时的时间）。
